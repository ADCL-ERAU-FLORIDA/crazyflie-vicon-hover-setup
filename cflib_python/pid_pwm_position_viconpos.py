...
class HostPIDPWMPositionController:
    ...
    def _wait_for_vicon(self, timeout_s: float):
        ...
        while time.monotonic() < end:
            if self._send_vicon_extpos():
                print("[Vicon] First extpos sent to EKF.")
                return
            time.sleep(0.01)
    ...
    def _reset_kalman_estimator(self):
        ...
        print(f"[EKF] Warming up with extpos for {EKF_WARMUP_S:.1f}s...")
        warmup_end = time.monotonic() + EKF_WARMUP_S
        while time.monotonic() < warmup_end:
            self._send_vicon_extpos()
            time.sleep(0.02)
        print("[EKF] Warmup complete.")

    def _send_vicon_extpos(self) -> bool:
        sample = self.vicon.get_latest()
        if not sample.valid:
            return False
