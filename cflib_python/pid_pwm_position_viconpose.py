...
class HostPIDPWMPositionController:
    def __init__(
        ...
        self.log_state.add_variable("gyro.x", "float")
        self.log_state.add_variable("gyro.y", "float")
        self.log_state.add_variable("gyro.z", "float")
    ...
    def run(self):
        ...
        with SyncCrazyflie(self.uri, cf=self.cf) as scf:
            ...
            try:
                ...
                self._wait_for_vicon(timeout_s=5.0)
                self._init_vicon_origin() 
                self._reset_kalman_estimator()
    ...
    def _wait_for_vicon(self, timeout_s: float):
        end = time.monotonic() + timeout_s
        while time.monotonic() < end:
            if self._send_vicon_extpose():
                print("[Vicon] First extpose sent to EKF.")
                return
            time.sleep(0.01)
        raise RuntimeError("Timed out waiting for valid Vicon pose")
        
    def _init_vicon_origin(self):
        sample = self.vicon.get_latest()
        if not sample.valid:
            raise RuntimeError("[Vicon] Cannot set origin — sample is not valid.")
        self.vicon_origin = (sample.x_m, sample.y_m, sample.z_m)
        print(f"[Vicon] Origin set: x={sample.x_m:.3f} y={sample.y_m:.3f} z={sample.z_m:.3f}")

    def _reset_kalman_estimator(self):
        print("[EKF] Enabling and resetting estimator...")
        # Force Crazyflie to use Kalman EKF.
        # This is needed so external pose measurements affect stateEstimate.
        self.cf.param.set_value("stabilizer.estimator", "2")
        time.sleep(0.1)
        self.cf.param.set_value("kalman.resetEstimation", "1")
        time.sleep(0.1)
        self.cf.param.set_value("kalman.resetEstimation", "0")
        print(f"[EKF] Warming up with extpose for {EKF_WARMUP_S:.1f}s...")
        warmup_end = time.monotonic() + EKF_WARMUP_S
        while time.monotonic() < warmup_end:
            self._send_vicon_extpose()
            time.sleep(0.02)
        print("[EKF] Warmup complete.")

    def _send_vicon_extpose(self) -> bool:
        sample = self.vicon.get_latest()
        if not sample.valid:
            return False
        q_norm = math.sqrt(
        ...
        if q_norm <= 1e-6:
            return False
        x0, y0, z0 = self.vicon_origin if self.vicon_origin else (0.0, 0.0, 0.0)
        x = sample.x_m - x0
        y = sample.y_m - y0
        z = sample.z_m - z0
        self.cf_state.position.x = x
        self.cf_state.position.y = y
        self.cf_state.position.z = z
    ...
    def _battery_compensator(
        ...
        #)
        motor_thrust_bat_comp.motor_1 = motor_thrust_uncapped.motor_1
        motor_thrust_bat_comp.motor_2 = motor_thrust_uncapped.motor_2
        motor_thrust_bat_comp.motor_3 = motor_thrust_uncapped.motor_3
        motor_thrust_bat_comp.motor_4 = motor_thrust_uncapped.motor_4
...
def parse_args():
    ...
    parser.add_argument(
        "--uri",
        default=uri_helper.uri_from_env(default="radio://0/100/2M/E7E7E7E701"),
        help="Crazyflie URI",
    )
    parser.add_argument(
        "--x",
